...
def analyze_dataset(data_dir: str):
    ...
    breed_folders = sorted(
        [f for f in data_path.iterdir() if f.is_dir() and not f.name.startswith(".")]
    )
...
def create_quick_test_dataset(source_dir: str, output_dir: str, max_breeds: int = 5):
    ...
    # Back to 5 breeds but with better selection (remove only the worst performers)
    # Keep: Australian_Shepherd_Dog (target), Japanese_spaniel (good), Siberian_husky (ok)
    # Remove: German_shepherd (4.2% - terrible), keep miniature_pinscher (but monitor)
    must_include = [
        "Australian_Shepherd_Dog",  # Target breed
        "Japanese_spaniel",  # Good performer (58.6%)
        "Siberian_husky",  # Decent (43.3%)
        "miniature_pinscher",  # Was dominating, but let's try with better training
        "Chihuahua",  # Replace German_shepherd with another breed
    ]
    selected_breeds = []
    ...
    for breed_name in must_include:
        if breed_name in breed_counts:
            ...
            print(
                f"🎯 FORCED inclusion: {breed_name} ({breed_counts[breed_name]} images)"
            )
        else:
    ...
    if remaining_breeds_needed > 0:
        ...
        available_breeds = [
            (name, count)
            for name, count in breed_counts.items()
        ...
        print(
            f"📊 Added {len(additional_breeds)} additional breeds to reach {max_breeds} total"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Prepare dataset for dog breed classification"
    )
    parser.add_argument(
        "--source",
        type=str,
    ...
    action_group.add_argument(
        "--split", action="store_true", help="Create train/val/test splits"
    )
    action_group.add_argument(
        "--analyze", action="store_true", help="Analyze dataset distribution"
    )
    action_group.add_argument(
        "--quick-test", action="store_true", help="Create small test dataset"
    )
