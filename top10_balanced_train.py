...
def top10_balanced_train():
    ...
    # Modello bilanciato per 10 classi
    model = create_breed_classifier(
        model_type='simple',
        num_classes=num_classes,
        dropout_rate=0.4  # Dropout moderato
    )
    ...
    optimizer = optim.Adam(
        model.parameters(),
        lr=learning_rate,
        weight_decay=5e-4  # Weight decay moderato
    )
    
    # Scheduler step-based più prevedibile
    scheduler = optim.lr_scheduler.StepLR(
        optimizer,
        step_size=5,   # Riduce ogni 5 epoche
        gamma=0.8      # Riduce del 20%
    )
    ...
    for i, (breed, acc, samples) in enumerate(final_class_accuracies, 1):
        ...
        print(f"   {status} {breed.replace('_', ' '):20s}: {acc:5.1f}% ({samples} samples){target_marker}")
