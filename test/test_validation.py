...
def load_model(model_path: str = "outputs/quick5/best_model.pth"):
    """Carica il modello addestrato (quick5 per default)"""
...
def test_validation(mode="all", model_path: str = "outputs/quick5/best_model.pth"):
    ...
    # Carica dataloaders da splits preorganizzati (5 razze)
    print("Caricando dataset da splits preorganizzati (data/breeds_5)...")
    train_loader, val_loader, test_loader = create_dataloaders_from_splits(
        splits_dir="data/breeds_5", batch_size=32, num_workers=2, image_size=(224, 224)
    )

    # Determina i nomi razze: dal checkpoint se presenti, altrimenti dal dataset
...
def test_sample_on_separated_set(model, test_loader, breed_names):
    ...
    if overall_accuracy >= 70:
        print(
            f"  ✅ PROSEGUI CON IL PROGETTO (Accuracy {overall_accuracy:.1f}% >= 70%)"
        )
    elif overall_accuracy >= 50:
        print(
            f"  ⚠️  PROSEGUI MA MIGLIORA IL MODELLO (Accuracy {overall_accuracy:.1f}%)"
        )
    else:
        print(
            f"  ❌ NON PROCEDERE - MIGLIORA PRIMA IL MODELLO (Accuracy {overall_accuracy:.1f}% < 50%)"
        )
...
def test_all_on_separated_set(model, test_loader, breed_names):
    ...
    for breed_name in breed_names:
        ...
        if stats["total"] > 0:
            ...
            print(
                f"🐕 {breed_name}: {accuracy:.1f}% accuracy, {avg_confidence:.1f}% avg confidence"
            )
